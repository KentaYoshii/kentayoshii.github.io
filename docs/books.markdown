  ...
  <div class="collection-items" data-items>
    <section class="year-block">
      <ul>
        ...
        <li data-title="{{ book.title | escape }}" data-author="{{ book.author | escape }}" data-letter="{{ book.letter }}"{% if book.year and book.year != "" %} data-date="{{ book.year }}"{% endif %}{% if book.pages %} data-pages="{{ book.pages }}"{% endif %}{% if book.published %} data-published="{{ book.published }}"{% endif %}{% if book.cover %} data-cover="{{ book.cover }}"{% endif %}{% if book.series %} data-series="{{ book.series | escape }}" data-series-index="{{ book.series_index }}"{% endif %}><em>{{ book.title | escape }}</em>{% if book.series %}<span class="entry-series">#{{ book.series_index }}</span>{% endif %}<span class="entry-byline">{{ book.author | escape }}</span>{% if book.year and book.year != "" %}<span class="entry-date"> · {{ book.year }}</span>{% endif %}</li>
        {%- endfor -%}
      </ul>
    </section>
  </div>
